...
- 时间复杂度：$O(\dfrac{2^n}{\sqrt{n}})$，其中 $n$ 是给定的**真二叉树**的结点数。只有当 $n$ 是奇数时才存在真二叉树，记 $2k+1$，由 $n$ 个结点组成的**真二叉树**的数量是第 $k$ 个卡特兰数 $\dfrac{C(2k, k)}{k + 1}$，其渐进上界为 $O(\dfrac{4^k}{k \sqrt{k}}) = O(\dfrac{2^n}{n \sqrt{n}})$，对于每个真二叉树需要 $O(n)$ 的时间生成和添加到答案中，因此时间复杂度是 $O(\dfrac{2^n}{\sqrt{n}})$。

- 空间复杂度：$O(n)$，其中 $n$ 是真二叉树的结点数。由于返回值不计入空间复杂度，此时递归深度最多为 $n$，递归调用栈需要 $O(n)$ 的空间。
...
- 时间复杂度：$O(\dfrac{2^n}{\sqrt{n}})$，其中 $n$ 是给定的**真二叉树**的结点数。只有当 $n$ 是奇数时才存在真二叉树，记 $2k+1$，由 $n$ 个结点组成的**真二叉树**的数量是第 $k$ 个卡特兰数 $\dfrac{C(2k, k)}{k + 1}$，其渐进上界为 $O(\dfrac{4^k}{k \sqrt{k}}) = O(\dfrac{2^n}{n \sqrt{n}})$，对于每个**真二叉树**至少需要求 $\dfrac{n}{2}$ 次卡特兰数，因此时间复杂度是 $O(\dfrac{2^n}{\sqrt{n}})$。
